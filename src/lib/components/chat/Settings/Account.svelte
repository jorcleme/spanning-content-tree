<script lang="ts">
	...
	const submitHandler = async () => {
		...
		const updatedUser = await updateUserProfile(localStorage.token, name, profileImageUrl).catch((error) => {
			toast.error(error);
		});
	...
	const createAPIKeyHandler = async () => {
		let res = await createAPIKey(localStorage.token);
		if (res) {
			APIKey = res.api_key;
	...
	const onFileInput = (e: Event & { currentTarget: EventTarget & HTMLInputElement }) => {
