<script lang="ts">
	import type { Collection, i18nType } from '$lib/types';
	...
	export let knowledge: Collection[] = [];

	let items: Collection[] = [];

	$: console.log('Knowledge/Selector -> items', items);
	$: console.log('Knowledge/Selector -> knowledge', knowledge);
				...
				? [
						{
							...
							collection_names: $documents.map((doc) => doc.collection_name) as string[]
						}
				  ]
				: []),
			...$documents
				...
				.map((tag) => ({
					...
					collection_names: $documents
						...
						.map((doc) => doc.collection_name) as string[]
				}))
		];
